...
def interact_with_user():
    ...
    while True:
        ...
        if user_choice == "1":
            ...
            average_salary = json_saver.calculate_average_salary(vacancies_list)
            print(f"Средняя зарплата по всем вакансиям: {average_salary}")

        elif user_choice == "3":
            top_count = int(input("Введите количество вакансий для вывода: "))
            sorted_vacancies = sorted(json_saver.get_vacancies_from_file(), key=lambda x: x.salary.get('from', 0),
                                      reverse=True)
